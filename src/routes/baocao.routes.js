...
router.get('/doanhthu', authenticateToken, checkRole('quan_ly'), async (req, res) => {
  const { startDate, endDate } = req.query;

  if (!startDate || !endDate) {
    return res.status(400).json({ error: 'Thiếu tham số startDate hoặc endDate' });
  }
...
router.get('/tonkho', authenticateToken, checkRole(['quan_ly', 'nhan_vien']), async (req, res) => {
  try {
...
router.get('/hieusuatnhanvien', authenticateToken, checkRole('quan_ly'), async (req, res) => {
  const { startDate, endDate } = req.query;
  ...
  try {
    ...
    res.status(500).json({ error: 'Lỗi khi lấy báo cáo hiệu suất nhân viên' });
  }
});
