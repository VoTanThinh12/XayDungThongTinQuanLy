...
async function main() {
  ...
  // Xóa toàn bộ dữ liệu cũ để tránh xung đột
  ...
  nhanViens.push(await prisma.nhan_vien.create({
    data: { 
      ho_ten: 'Quản Lý Chính', tai_khoan: 'admin', mat_khau: hashedPassword, vai_tro: 'quan_ly', trang_thai: true, 
      email: 'admin@example.com', so_dien_thoai: '0987654321' 
    }
  }));
  nhanViens.push(await prisma.nhan_vien.create({
    data: { 
      ho_ten: 'Trần Thu Ngân', tai_khoan: 'thungan', mat_khau: hashedPassword, vai_tro: 'thu_ngan', trang_thai: true, 
      email: 'thungan@example.com', so_dien_thoai: '0912345679' 
    }
  }));
  nhanViens.push(await prisma.nhan_vien.create({
    data: { 
      ho_ten: 'Lê Khoa Học', tai_khoan: 'nhanvienkho', mat_khau: hashedPassword, vai_tro: 'nhan_vien_kho', trang_thai: true, 
      email: 'nhanvienkho@example.com', so_dien_thoai: '0912345680' 
    }
  }));
  ...
  const phieuNhaps = [];
  const nhanVienKhoList = nhanViens.filter(nv => nv.vai_tro === 'nhan_vien_kho');
  for (let i = 1; i <= 50; i++) {
    ...
    for (let j = 0; j < soLuongChiTiet; j++) {
      ...
      chiTietNhap.push({
        ...
        gia_nhap: sanPham.gia_ban - getRandomInt(500, 2000),
      });
    }
  ...
  const hoaDonBans = [];
  const thuNganList = nhanViens.filter(nv => nv.vai_tro === 'thu_ngan');
  for (let i = 1; i <= 50; i++) {
    ...
    for (let j = 0; j < soLuongChiTiet; j++) {
      ...
      tongTien += soLuong * sanPham.gia_ban;
    }
