...
const EmployeePerformanceReport = memo(() => {
  ...
  const fetchPerformanceReport = useCallback(async (dates) => {
    ...
    try {
      ...
      if (response.data && Array.isArray(response.data)) {
        ...
        message.success(`Đã tạo báo cáo hiệu suất cho ${response.data.length} nhân viên`);
      } else {
        setPerformanceData([]);
  ...
  const getTotalRevenue = useCallback((record) => {
    return record?._sum?.tong_tien || record?.tong_doanh_thu || 0;
  }, []);
  ...
  const totals = performanceData.reduce((acc, record) => {
    acc.totalInvoices += getInvoiceCount(record);
    acc.totalRevenue += getTotalRevenue(record);
    return acc;
